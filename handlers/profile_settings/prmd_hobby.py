...
prmd_hobby_router = DefaultRouter()


@simple_bot_handler(prmd_hobby_router, None, filters.EventTypeFilter('message_event'),
                    filters.PayloadFilter({'command': 'back'}),
                    StateFilter(fsm=fsm, state=Profile.d_m_hobby, for_what=ForWhat.FOR_USER))
...
@simple_bot_message_handler(prmd_hobby_router,
                            StateFilter(fsm=fsm, state=Profile.d_m_hobby, for_what=ForWhat.FOR_USER))
